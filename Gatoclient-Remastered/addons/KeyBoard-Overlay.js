const addonInfo = {
    ...
    description: "KeyBoard OverLay Config by DuyHungDev",
...
class gatoAddon {
    ...
    static initialize(dependencies) {
        ...
        console.log("KeyBoard Overlay By DuyHungDev");
    }
    ...
    static loadAddonSettings(dependencies) {
        ...
        addonSetUtils.createCheckbox(addonInfo["id"], "enabled", "[Bật/Tắt] Addon", "Reload lại krunker để hiệu nhiệm", "addonSettings", false, 2);

        addonSetUtils.createCategory("visualSettings", "Visual Settings");
        addonSetUtils.createSlider(addonInfo["id"], "leftPercent", "Độ X (Mặc định 8%)", "Trái phải (Thấp = trái và Cao = phải)", 0, 100, 50, 1, "visualSettings", false);
        addonSetUtils.createSlider(addonInfo["id"], "topPercent", "Độ Y (Mặc định 24%)", "Lên xuống (Thấp = lên và Cao = xuống)", 0, 100, 90, 1, "visualSettings", false);
        addonSetUtils.createSlider(addonInfo["id"], "scale", "Độ rộng (Mặc định 1,1%)", "Đổi độ rộng của Overlay", 0.1, 2.0, 0.5, 0.1, "visualSettings", false);
        ...
        function openOriginal() {
            window.open("https://fb.com/duyhung.json");
        }
        addonSetUtils.createButton(addonInfo["id"], "openOriginal", "[Credit]", "Làm bởi DuyHungDev", "[FB]", openOriginal, "addonSettings", false);
